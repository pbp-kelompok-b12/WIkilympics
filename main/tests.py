...
class HomeViewTest(TestCase):
    ...
    def test_home_view_requires_login(self):
        response = self.client.get(reverse('threads:home'))
...
class EditForumViewTest(TestCase):
    ...
    def test_edit_forum_requires_login(self):
        response = self.client.get(reverse('threads:edit_forum', args=[self.forum.id]))
        self.assertEqual(response.status_code, 302)
    ...
    def test_edit_forum_non_owner_redirected(self):
        ...
        response = self.client.get(reverse('threads:edit_forum', args=[self.forum.id]))
        self.assertRedirects(response, reverse('threads:home'))
    
    def test_edit_forum_post(self):
        self.client.login(username='testuser', password='testpass123')
        response = self.client.post(reverse('threads:edit_forum', args=[self.forum.id]), {
            'topic': 'Updated Forum',
...
class EditDiscussionViewTest(TestCase):
    ...
    def test_edit_discussion_requires_login(self):
        response = self.client.get(reverse('threads:edit_discussion', args=[self.discussion.id]))
        self.assertEqual(response.status_code, 302)
...
class AddInForumViewTest(TestCase):
    ...
    def test_add_forum_requires_login(self):
        response = self.client.get(reverse('threads:addInForum'))
        self.assertEqual(response.status_code, 302)
...
class AddInDiscussionViewTest(TestCase):
    ...
    def test_add_discussion_requires_login(self):
        response = self.client.get(reverse('threads:addInDiscussion', args=[self.forum.id]))
        self.assertEqual(response.status_code, 302)
    ...
    def test_add_discussion_post(self):
        self.client.login(username='testuser', password='testpass123')
        response = self.client.post(
            reverse('threads:addInDiscussion', args=[self.forum.id]),
            {
                'forum': self.forum.id,
...
class DeleteForumViewTest(TestCase):
    ...
    def test_delete_forum_requires_login(self):
        response = self.client.post(reverse('threads:delete_forum', args=[self.forum.id]))
        self.assertEqual(response.status_code, 302)
...
class DeleteDiscussionViewTest(TestCase):
    ...
    def test_delete_discussion_requires_login(self):
        response = self.client.post(reverse('threads:delete_discussion', args=[self.discussion.id]))
        self.assertEqual(response.status_code, 302)
...
class RegisterViewTest(TestCase):
    ...
    def test_register_get(self):
        response = self.client.get(reverse('threads:register'))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'register.html')
    
    def test_register_post_valid(self):
        response = self.client.post(reverse('threads:register'), {
            'username': 'newuser',
            'password1': 'complexpass123',
            'password2': 'complexpass123'
        })
        self.assertEqual(User.objects.count(), 1)
        self.assertRedirects(response, reverse('threads:login'))
...
class LoginViewTest(TestCase):
    ...
    def test_login_get(self):
        response = self.client.get(reverse('threads:login'))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'login.html')
    
    def test_login_post_valid(self):
        response = self.client.post(reverse('threads:login'), {
            'username': 'testuser',
            'password': 'testpass123'
        })
        self.assertRedirects(response, reverse('threads:show_main'))
...
class LogoutViewTest(TestCase):
    ...
    def test_logout(self):
        self.client.login(username='testuser', password='testpass123')
        response = self.client.get(reverse('threads:logout'))
        self.assertRedirects(response, reverse('threads:login'))
